...
class CuriosityManager {
  ...
  async fetch(domain = "general") {
    ...
    try {
      ...
      if (!knowledgeDomain) {
        // Return random facts from all domains
        const allFacts = Object.values(this.knowledgeBase).flatMap((d) => d.facts);
        const shuffled = allFacts.sort(() => 0.5 - Math.random()).slice(0, 5);

        const result = {
          source: "curiosity",
          domain: "general",
          facts: shuffled,
          description: "Curated science facts from multiple domains",
          timestamp: new Date().toISOString(),
        };
...
class DataManagers {
  ...
  getStatus() {
    return {
      name: this.name,
      emoji: this.emoji,
      managers: Object.entries(this.managers).map(([key, manager]) => ({
        name: manager.name,
